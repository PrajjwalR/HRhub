  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Step 2: Fetch Leave Types, Holidays, and Current Leave Data
        const [typesRes, holidaysRes, leaveRes] = await Promise.all([
          fetch("/api/leave-types"),
          fetch(`/api/holidays?employee=${employeeId}`),
          fetch(`/api/leaves?id=${id}`)
        ]);
        ...
        if (!leaveRes.ok) throw new Error("Failed to fetch leave details");
        const currentLeave = await leaveRes.json();
        
        setFormData({
          leave_type: currentLeave.leave_type,
          from_date: currentLeave.from_date,
          to_date: currentLeave.to_date,
          reason: currentLeave.reason || "",
          half_day: !!currentLeave.half_day,
        });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.error || "Failed to update leave application");
      }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#FAFAFA]">
        <div className="w-8 h-8 border-4 border-[#FF8C42]/30 border-t-[#FF8C42] rounded-full animate-spin"></div>
      </div>
    );
  }
  ...
  return (
    <div className="px-8 py-6 bg-[#FAFAFA] min-h-screen">
      <div className="max-w-3xl mx-auto">
        ...
        <h1 className="text-3xl font-serif text-[#2C2C2C] mb-2">Edit Leave Application</h1>
        <p className="text-gray-500 mb-8">Update the details of your requested time off.</p>
